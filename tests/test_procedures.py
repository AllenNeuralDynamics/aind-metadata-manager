...
from aind_metadata_manager.upgrade.procedures_upgrade import ProcedureUpgrade

PYD_VERSION = re.match(r"(\d+.\d+).\d+", pyd_version).group(1)

PROCEDURES_FILES_PATH = (
    Path(__file__).parent / "resources" / "procedures" / "class_model_examples"
)
PYD_VERSION = re.match(r"(\d+.\d+).\d+", pyd_version).group(1)

log_file_name = (
    "./tests/resources/procedures/log_files/log_"
...
class TestProceduresUpgrade(unittest.TestCase):
    ...
    def test_upgrade_procedure(self):
        ...
        for file, procedure in self.procedures.items():
            ...
            ProcedureUpgrader = ProcedureUpgrade(
                procedure, allow_validation_errors=True
            )
    ...
    def test_probe_upgrade(self):
        ...
        self.assertEqual(
            p.subject_procedures[1]
            .procedures[2]
            .probes[0]
            .ophys_probe.core_diameter_unit,
            "um",
        )
        self.assertEqual(
