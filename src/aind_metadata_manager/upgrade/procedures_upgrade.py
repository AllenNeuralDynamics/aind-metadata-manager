...
from aind_metadata_manager.upgrade.base_upgrade import BaseModelUpgrade
from aind_metadata_manager.upgrade.utils import (
    construct_new_model,
    get_or_default,
)
...
class InjectionMaterialsUpgrade:
    ...
    def upgrade_viral_material(self, material: dict) -> ViralMaterial:
        ...
        return construct_new_model(
            viral_dict, ViralMaterial, self.allow_validation_errors
        )
    ...
    def upgrade_nonviral_material(self, material: dict) -> NonViralMaterial:
        ...
        nonviral_dict = {
            ...
            "concentration_unit": get_or_default(
                material, NonViralMaterial, "concentration_unit"
            ),
            "name": material.get("name", "unknown"),
            ...
            "expiration_date": get_or_default(
                material, NonViralMaterial, "expiration_date"
            ),
        }

        return construct_new_model(
            nonviral_dict, NonViralMaterial, self.allow_validation_errors
        )

    def upgrade_injection_materials(
        self, old_injection_materials: list
    ) -> Optional[dict]:
        ...
        for injection_material in old_injection_materials:
            ...
            if dict(injection_material).get("titer") is not None:
                new_materials.append(
                    self.upgrade_viral_material(injection_material)
                )

            elif injection_material.get("concentration") is not None:
                new_materials.append(
                    self.upgrade_nonviral_material(injection_material)
                )
            else:
                logger.error(
                    "Injection material with no titer or concentration "
                    f"{injection_material} passed in"
                )
...
class SubjectProcedureModelsUpgrade(BaseModelUpgrade):
    ...
    def __init__(self, allow_validation_errors=False):
        ...
        self.allow_validation_errors = allow_validation_errors
        logging.info(
            "ALLOW VALIDATION ERRORS (SUBJ PROCEDURES): "
            f"{self.allow_validation_errors}"
        )
        self.injection_upgrader = InjectionMaterialsUpgrade(
    ...
    def upgrade_craniotomy(self, old_subj_procedure: dict):
        ...
        craniotomy_dict = {
            ...
            "craniotomy_hemisphere": get_or_default(
                old_subj_procedure, Craniotomy, "craniotomy_hemisphere"
            ),
            "bregma_to_lambda_distance": old_subj_procedure.get(
                "bregma_to_lambda_distance", None
            ),
            "bregma_to_lambda_unit": get_or_default(
                old_subj_procedure, Craniotomy, "bregma_to_lambda_unit"
            ),
            "implant_part_number": old_subj_procedure.get(
                "implant_part_number", None
            ),
            "dura_removed": get_or_default(
                old_subj_procedure, Craniotomy, "dura_removed"
            ),
            "protective_material": get_or_default(
                old_subj_procedure, Craniotomy, "protective_material"
            ),
            "recovery_time": old_subj_procedure.get("recovery_time", None),
            "recovery_time_unit": get_or_default(
                old_subj_procedure, Craniotomy, "recovery_time_unit"
            ),
        }
        ...
        if craniotomy_dict["craniotomy_type"] in ["3mm", "5mm"]:
            craniotomy_dict["craniotomy_type"] = craniotomy_dict[
                "craniotomy_type"
            ].replace("mm", " mm")
        ...
        return construct_new_model(
            craniotomy_dict, Craniotomy, self.allow_validation_errors
        )
    ...
    def construct_ophys_probe(self, probe: dict):
        ...
        fiber_probe_dict = {
            ...
            "core_diameter_unit": get_or_default(
                probe, FiberProbe, "core_diameter_unit"
            ).replace("μm", "um"),
            "numerical_aperture": probe.get("numerical_aperture", None),
            "ferrule_material": get_or_default(
                probe, FiberProbe, "ferrule_material"
            ),
        ...
        ophys_probe_dict = {
            "ophys_probe": construct_new_model(
                fiber_probe_dict, FiberProbe, self.allow_validation_errors
            ),
            "targeted_structure": probe.get("targeted_structure", "unknown"),
            "stereotactic_coordinate_ap": Decimal(
                probe.get("stereotactic_coordinate_ap", None)
            ),
            "stereotactic_coordinate_ml": Decimal(
                probe.get("stereotactic_coordinate_ml", None)
            ),
            "stereotactic_coordinate_dv": Decimal(
                probe.get("stereotactic_coordinate_dv", None)
            ),
            "stereotactic_coordinate_unit": get_or_default(
                probe, OphysProbe, "stereotactic_coordinate_unit"
            ),
            "stereotactic_coordinate_reference": probe.get(
                "stereotactic_coordinate_reference", None
            ),
            "bregma_to_lambda_distance": probe.get(
                "bregma_to_lambda_distance", None
            ),
            "bregma_to_lambda_unit": get_or_default(
                probe, OphysProbe, "bregma_to_lambda_unit"
            ),
        ...
        return construct_new_model(
            ophys_probe_dict, OphysProbe, self.allow_validation_errors
        )
    ...
    def upgrade_fiber_implant(self, old_subj_procedure: dict):
        ...
        return construct_new_model(
            fiber_implant_dict, FiberImplant, self.allow_validation_errors
        )

    def add_probe(
        self, old_subj_procedure: dict, fiber_implant_model: FiberImplant
    ):
        ...
        if type(old_subj_procedure["probes"]) is list:
            for probe in old_subj_procedure["probes"]:
                ...
                fiber_implant_model = fiber_implant_model.probes.append(
                    self.construct_ophys_probe(probe)
                )
    ...
    def upgrade_headframe(self, old_subj_procedure: dict):
        ...
        headframe_dict = {
            "protocol_id": old_subj_procedure.get("protocol_id", "unknown"),
            "headframe_part_number": old_subj_procedure.get(
                "headframe_part_number", "unknown"
            ),
            "headframe_type": old_subj_procedure.get(
                "headframe_type", "unknown"
            ),
            "headframe_material": get_or_default(
                old_subj_procedure, Headframe, "headframe_material"
            ),
            "well_part_number": get_or_default(
                old_subj_procedure, Headframe, "well_part_number"
            ),
            "well_type": get_or_default(
                old_subj_procedure, Headframe, "well_type"
            ),
        }

        return construct_new_model(
            headframe_dict, Headframe, self.allow_validation_errors
        )
    ...
    def upgrade_nanoject_injection(self, old_subj_procedure: dict):
        ...
        injection_dict = {
            "injection_volume": old_subj_procedure.get(
                "injection_volume", None
            ),
            "injection_volume_unit": get_or_default(
                old_subj_procedure, NanojectInjection, "injection_volume_unit"
            ),
            "injection_materials": old_subj_procedure.get(
                "injection_materials", [None]
            ),
            "recovery_time": old_subj_procedure.get("recovery_time", None),
            "recovery_time_unit": get_or_default(
                old_subj_procedure, NanojectInjection, "recovery_time_unit"
            ),
            "injection_duration": old_subj_procedure.get(
            ...
            "protocol_id": old_subj_procedure.get(
                "protocol_id", "dx.doi.org/10.17504/protocols.io.bgpujvnw"
            ),
            "injection_coordinate_ml": old_subj_procedure.get(
                "injection_coordinate_ml", None
            ),
            "injection_coordinate_ap": old_subj_procedure.get(
                "injection_coordinate_ap", None
            ),
            "injection_coordinate_depth": old_subj_procedure.get(
            ...
            "injection_coordinate_reference": old_subj_procedure.get(
            ...
            "bregma_to_lambda_unit": get_or_default(
                old_subj_procedure, NanojectInjection, "bregma_to_lambda_unit"
            ),
            "injection_angle": old_subj_procedure.get("injection_angle", None),
            "injection_angle_unit": get_or_default(
                old_subj_procedure, NanojectInjection, "injection_angle_unit"
            ),
            "targeted_structure": old_subj_procedure.get(
                "targeted_structure", None
            ),
            "injection_hemisphere": old_subj_procedure.get(
                "injection_hemisphere", None
            ),
        }

        return construct_new_model(
            injection_dict, NanojectInjection, self.allow_validation_errors
        )
    ...
    def upgrade_perfusion(self, old_subj_procedure: dict):
        ...
        perfusion_dict = {
            "protocol_id": old_subj_procedure.get(
                "protocol_id", self.DEFAULT_PERFUSION_PROTOCOL
            ),
            "output_specimen_ids": [
                str(item)
                for item in old_subj_procedure.get("output_specimen_ids", [])
            ],
        }

        model = construct_new_model(
            perfusion_dict, Perfusion, self.allow_validation_errors
        )
    ...
    def upgrade_retro_orbital_injection(self, old_subj_procedure: dict):
        ...
        retro_orbital_dict = {
            ...
            "injection_eye": old_subj_procedure.get(
                "injection_eye", "unknown"
            ),
            "injection_materials": old_subj_procedure.get(
                "injection_materials", None
            ),
            "recovery_time": old_subj_procedure.get("recovery_time", None),
            "recovery_time_unit": get_or_default(
                old_subj_procedure, RetroOrbitalInjection, "recovery_time_unit"
            ),
            "injection_duration": old_subj_procedure.get(
                "injection_duration", None
            ),
...
def set_craniotomy_type(
    surgery: Surgery,
):  # find a better organizational place for this
    """Set the craniotomy type based on the headframe type"""

    craniotomy = [x for x in surgery.procedures if isinstance(x, Craniotomy)][
        0
    ]
    if any(isinstance(x, Headframe) for x in surgery.procedures):

        headframe = [
            x for x in surgery.procedures if isinstance(x, Headframe)
        ][0]
...
class ProcedureUpgrade(BaseModelUpgrade):
    ...
    def __init__(
        self,
        old_procedures_dict: Union[dict, Procedures],
        ...
        super().__init__(
            old_procedures_dict,
            model_class=Procedures,
            allow_validation_errors=allow_validation_errors,
        )

        self.subj_procedure_upgrader = SubjectProcedureModelsUpgrade(
            allow_validation_errors
        )
        logging.info(
            "ALLOW VALIDATION ERRORS: "
            f"{self.subj_procedure_upgrader.allow_validation_errors}"
        )

        self.upgrade_funcs = {
            "Craniotomy": self.subj_procedure_upgrader.upgrade_craniotomy,
            "Fiber implant": self.subj_procedure_upgrader.upgrade_fiber_implant,  # noqa: E501
            "Headframe": self.subj_procedure_upgrader.upgrade_headframe,
            "Nanoject injection": self.subj_procedure_upgrader.upgrade_nanoject_injection,  # noqa: E501
            "Perfusion": self.subj_procedure_upgrader.upgrade_perfusion,
            "Retro-orbital injection": (
                self.subj_procedure_upgrader.upgrade_retro_orbital_injection
            ),
        }
    ...
    def upgrade_subject_procedure(self, old_subj_procedure: dict):
        ...
        if procedure_type in self.upgrade_funcs.keys():
            ...
            if old_subj_procedure.get("injection_materials"):
                old_subj_procedure[
                    "injection_materials"
                ] = InjectionMaterialsUpgrade(
                    self.allow_validation_errors
                ).upgrade_injection_materials(
                    old_subj_procedure["injection_materials"]
                )
            else:
                old_subj_procedure["injection_materials"] = [None]

            return self.caller(
                self.upgrade_funcs[procedure_type], old_subj_procedure
            )
        else:
            logger.error(
                f"Procedure type {procedure_type} not found in list "
                "of procedure types"
            )
            return None
    ...
    def upgrade_procedure(self) -> Optional[Procedures]:
        ...
        if semver.Version.parse(
            ...
            for subj_procedure in self.old_model_dict.get(
                "subject_procedures"
            ):  # type: dict
                ...
                logger.info(
                    "Upgrading procedure %s for subject %s on date %s",
                    subj_procedure.get("procedure_type"),
                    subj_id,
                    date,
                )
                logger.info(f"Old procedure: {subj_procedure}")

                if date not in loaded_subject_procedures.keys():
                    logger.info(
                        f"Creating new surgery for subject {subj_id} on date "
                        f"{date}"
                    )

                    subj_procedures = [
                        self.upgrade_subject_procedure(
                            old_subj_procedure=subj_procedure
                        )
                    ]
                    ...
                    new_surgery_dict = {
                        ...
                        "experimenter_full_name": str(
                            subj_procedure.get("experimenter_full_name")
                        ),
                        "iacuc_protocol": subj_procedure.get("iacuc_protocol"),
                        "animal_weight_prior": subj_procedure.get(
                            "animal_weight_prior"
                        ),
                        "animal_weight_post": subj_procedure.get(
                            "animal_weight_post"
                        ),
                        "weight_unit": subj_procedure.get(
                            "weight_unit",
                            Surgery.model_fields["weight_unit"].default,
                        ),
                        ...
                        "protocol_id": subj_procedure.get(
                            "protocol_id", "unknown"
                        ),
                    }
                    logger.info(f"new surgery: {new_surgery_dict}")
                    loaded_subject_procedures[date] = new_surgery_dict
                else:
                    logger.info(
                        "Adding procedure %s for subject %s on date %s",
                        subj_procedure.get("procedure_type"),
                        subj_id,
                        date,
                    )

                    if subj_procedure.get(
                        "procedure_type"
                    ) == "Fiber implant" and any(
                        isinstance(x, FiberImplant)
                        for x in loaded_subject_procedures[date]["procedures"]
                    ):
                        logger.info(
                            "Adding probe to existing fiber implant for subject %s on date %s",  # noqa: E501
                            subj_id,
                            date,
                        )
                        for x in loaded_subject_procedures[date]["procedures"]:
                            if isinstance(x, FiberImplant):
                                ...
                                SubjectProcedureModelsUpgrade(
                                    allow_validation_errors=self.allow_validation_errors  # noqa: E501
                                ).add_probe(subj_procedure, x)

                    else:
                        logger.info(
                            f"Adding procedure to existing surgery for subject {subj_id}"  # noqa: E501
                            f"on date {date}: {subj_procedure}"
                        )
                        logger.info(
                            f"existing surgery: {loaded_subject_procedures[date]}"  # noqa: E501
                        )
            ...
            constructed_subject_procedures = {
                date: construct_new_model(
                    surgery, Surgery, self.allow_validation_errors
                )
            ...
            for surgery in constructed_subject_procedures.values():
                logging.info(
                    f"Setting craniotomy type for subject {subj_id}, "
                    f"surgery: {surgery}"
                )
            ...
            # NOTE: Current set lacks specimen procedures.
            # Can be returned in the future.
            # for spec_procedure in self.old_model_dict.specimen_procedures:
            #     date = spec_procedure.start_date
            #     logging.info(
            #         f"Upgrading procedure {spec_procedure.get('procedure_type')} "  # noqa: E501
            #         f"for subject {subj_id} on date {date}"
            #     )
            #     upgraded_spec_procedure = ProcedureUpgrade.upgrade_specimen_procedure(  # noqa: E501
            #         spec_procedure
            #     )
            ...
            logging.info(
                "constructed Subject procedures: "
                f"{constructed_subject_procedures.values()}"
            )
            ...
            return construct_new_model(
                self.old_model_dict, Procedures, self.allow_validation_errors
            )
