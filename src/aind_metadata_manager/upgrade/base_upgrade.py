...
class BaseModelUpgrade(ABC):
    ...
    def __init__(
        self,
        old_model: Union[dict, AindModel],
    ...
    def _get_or_default(
        self, model: dict, field_name: str, kwargs: dict
    ) -> Any:
        """
        If field is not explicitly set, will attempt to extract from a model.
        If field is not found in old model, will attempt to set using the
        default.
        ...
        if kwargs.get(field_name) is not None:
            ...
            and field_name in model.keys()
            and model.get(field_name) is not None
        ):
            return model.get(field_name)
        else:
            try:
                attr_default = getattr(
                    self.model_class.model_fields.get(field_name), "default"
                )
