...
class DataProcessUpgrade(BaseModelUpgrade):
    ...
    def __init__(self, old_data_process_dict: Union[dict, DataProcess]):
        ...
        """
        super().__init__(
            old_model=old_data_process_dict, model_class=DataProcess
        )
    ...
    def upgrade(self, **kwargs) -> AindModel:
        ...
        software_version = self._get_or_default(
            self.old_model_dict, "software_version", kwargs
        )
        ...
        # this takes care of setting the outputs to an empty dict
        # (default) if it is None
        self.old_model_dict["outputs"] = outputs

        return construct_new_model(
            self.old_model_dict, DataProcess, self.allow_validation_errors
        )
...
class ProcessingUpgrade(BaseModelUpgrade):
    ...
    def __init__(self, old_processing_model: Processing):
        ...
        """
        super().__init__(
            old_model=old_processing_model, model_class=Processing
        )
    ...
    def upgrade(self, **kwargs) -> AindModel:
        ...
        if schema_version is None or schema_version < "0.3.0":
            data_processes = self._get_or_default(
                self.old_model_dict, "data_processes", kwargs
            )
            pipeline_version = self._get_or_default(
                self.old_model_dict, "pipeline_version", kwargs
            )
            pipeline_url = self._get_or_default(
                self.old_model_dict, "pipeline_version", kwargs
            )
            ...
            processing_pipeline = self._get_or_default(
                self.old_model_dict, "processing_pipeline", kwargs
            )
            # upgrade data processes
            data_processes_new = []
            ...
            for data_process in data_processes_old:
                data_processes_new.append(
                    DataProcessUpgrade(data_process).upgrade()
                )
            processing_pipeline_dict.pop("data_processes")
            processing_pipeline = PipelineProcess(
                data_processes=data_processes_new, **processing_pipeline_dict
            )

        return Processing(
            processing_pipeline=processing_pipeline,
            analyses=self._get_or_default(
                self.old_model_dict, "analyses", kwargs
            ),
            notes=self._get_or_default(self.old_model_dict, "notes", kwargs),
        )
