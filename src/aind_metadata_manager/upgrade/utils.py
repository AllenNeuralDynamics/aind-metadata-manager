...
def construct_new_model(
    ...
    """Validate a model, if it fails and validation error
    flag is on, construct a model
    """
...
def get_or_default(model: dict, model_type: AindModel, field_name: str):
    """Version of get_or_default that works with a dict instead
    of a model instance.
    ...
    if model.get(field_name, None) is not None:
        ...
        try:
            attr_default = getattr(
                model_type.model_fields.get(field_name), "default"
            )
